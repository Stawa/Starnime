...
export class DiscordEmbed {
    ...
    __parseColor(color: string | number[]): number {
        ...
        if (Array.isArray(color))
            return (
                (color[0] || 0 << 16) + (color[1] || 0 << 8) + (color[2] || 0)
            );
        if (/^#?[\da-f]{6}$/i.test(color))
            return parseInt(color.replace("#", ""), 16);

        throw new Error(
            "Invalid color value. The color format is not recognized.",
        );
    }
}
...
export class DiscordWebhook {
    ...
    constructor(options: DiscordWebhookOptions) {
        this.options =
            options instanceof DiscordWebhookOptions
                ? options
                : new DiscordWebhookOptions(options);
        this.payload = {
            embeds: [],
        };
    }
