...
class DiscordEmbed {
	...
	__parseColor(color) {
		if (typeof color === 'number') {
			if (color < 0 || color > 0xffffff) throw new Error('Invalid color value. The color should be a number between 0 and 0xffffff.');
		}

		if (Number.isNaN(color)) throw new Error('Invalid color value. The color should be a valid number.');
