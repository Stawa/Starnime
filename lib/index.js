...
class Starnime {
	...
	async neko(outputType) {
		return await this.image(this.options.defaultVersion, Category.SFW, EndpointSfw.NEKO, outputType || this.options.outputType)
	}
	...
	async bite(outputType) {
		return await this.image(this.options.defaultVersion, Category.SFW, EndpointSfw.BITE, outputType || this.options.outputType)
	}
	 ...
	 * @throws {StarError} If the API request fails or encounters an error.
	 */
	async image(version, category, endpoint, outputType) {
		const fetch = await request.get(version, category, endpoint, outputType);
